...
sobel_Kernel () {
  ...
  __shared__ int d_mask[MASK_N*MASK_X*MASK_Y];
  if(id<MASK_N*MASK_X*MASK_Y)
  {
      cudaMemcpy((void**)&&d_mask[id], sizeof(int));
  }
  __syncthreads();
...
main(int argc, char **argv) {
  ...
  size_t image_size = sizeof(unsigned char)* (width * height * (long)byte_per_pixel);
  // cudaMalloc((void**)&d_mask, mask_size);
  cudaMalloc((void**)&d_image_s, image_size);

  cudaMemcpy(d_mask, mask, mask_size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_image_s, image_s, image_size, cudaMemcpyHostToDevice);
  sobel_Kernel<<1,1>>();
	// Task 1: Memory Copy from Device (GPU) to Host
	// Hint  : cudaMemcpy ( ... , cudaMemcpyDeviceToHost )
  cudaMemcpy(image_t, d_image_t, image_size, cudaMemcpyDeviceToHost);
